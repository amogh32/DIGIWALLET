<?php
	...
	if($email == $emailid){
		...
		echo "<script>window.location.href='minedebt.php'</script>";		
	}
	else{
		...
		if(mysqli_num_rows($result1) == 0){
			...
			$qry2 = "INSERT INTO `debt` (`Email1`, `Email2`, `Money`, `Confirm`,`Mine`) VALUES ('$emailid', '$email', '$amount', 0, 1)";
			
			if(mysqli_query($con,$qry2)){
				echo "<script>alert('New Mine Debt Request Sent')</script>";
				echo "<script>window.location.href='minedebt.php'</script>";		
			}
			else{
