...
<body>
	<div class="topnav" style="margin-left:350px;padding:12px">
		...
		<a href="minedebt.php" class="apart">Mine debt</a>
		<a href="pending_request.php"  class="apart">Pending Request</a>
		<a href="myborrow.php" class="active" class="apart" >My borrow</a>
...
<?php
	...
	$qry = "SELECT * FROM debt WHERE ((Email2 = '$emailid' AND Mine = 0) OR (Email1 = '$emailid' AND Mine = 1)) AND Confirm = 1";

	$result = mysqli_query($con,$qry);

	if(mysqli_num_rows($result) == 0){
		echo "<p>No borrow requests</p>";
	}
	else{
		...
		while($row = mysqli_fetch_assoc($result)){
			...
			echo '<tr>';
				...
				if($row["Mine"]==1){
					...
					echo '<td><a href="payborrow.php?id=' .$id. '"><button class="approvebtn">Pay</button></a></td>';
					echo '<td><a href="delete_request.php?id=' .$id. '"><button class="removebtn">Remove</button></a></td>';
				}
				else{
